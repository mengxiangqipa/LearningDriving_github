 ...
 *
 */
public abstract class ImageLoaderUtils_nostra13 {
	// 使用方法
	// //
	// ImageLoaderUtils_nostra13.initImageLoader(getActivity().getApplicationContext()).displayImage(nl.thumbnail.split(";")[0],
	// //
	// (ImageView)viewHolder.getView(R.id.listitem_iv_icon),ToolImage.getFadeOptions(R.drawable.bg_default_listitem_icon,R.drawable.ic_launcher,R.drawable.allview_load_failed));
	...
	public static ImageLoader initImageLoader(Context context) {
		Y.i("initImageLoader_1");
		// 获取到缓存的目录地址
		File cacheDir = StorageUtils.getOwnCacheDirectory(context,
				"ImageLoader/Cache");
		Y.i("initImageLoader_2");
		...
		ImageLoaderConfiguration config = new ImageLoaderConfiguration.Builder(
				...
				.build();
		Y.i("initImageLoader_3");
		// 全局初始化此配置
		imageLoader = ImageLoader.getInstance();
		Y.i("initImageLoader_4");
		imageLoader.init(config);
		Y.i("initImageLoader_5");
		return imageLoader;
	}
	...
	public static DisplayImageOptions getFadeOptions(int loadingImageResId,
		...
		// 设置图片在下载期间显示的图片
				 ...
				 **/
				.displayer(
						new FadeInBitmapDisplayer(1 * 1000, false, false, true))
				// .displayer(new FadeInBitmapDisplayer(1*1000))// 渐显--设置图片渐显的时间
				.build();
		Y.i("getFadeOptions_2");
		return options;
	}
