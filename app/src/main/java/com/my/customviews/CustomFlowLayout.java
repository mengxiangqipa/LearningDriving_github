...
/**
 * @author Yobert Jomi
 *         className CustomFlowLayout 流式布局
 *         created at  2016/7/20  13:28
 */
public class CustomFlowLayout extends LinearLayout {
    /**
     * 存储所有的子View
     */
    private List<List<View>> listAllChildViews;
    /**
     * destination:容器 --装载LinearLayout
     */
    private List<LinearLayout> listContainer;
    private List<View> listLineViews;
    /**
     * 存储每一行的高度
     */
     ...
     * <pre>
     *     @param views 数组
     *     @return checkable 布局为checkbox时，设置为true可响应oncheckchangedlistener
...
//         记录当前行的view
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child instanceof CheckBox && checkable) {
                child.setId(10000 + i);
                ((CheckBox) child).setOnCheckedChangeListener(new MyOnCheckedChangeListener());
            } else if (child instanceof CheckBox && !checkable) {
                ((CheckBox) child).setEnabled(false);
            }
            ...
            if (childWidth + lineWidth + lp.leftMargin + lp.rightMargin > width) {
                ...
                //自己添加
//                View viewTemp=child;
//                linearLayout.setId(10000 + listAllChildViews.size());
//                linearLayout.addView(viewTemp);
//                listContainer.add(linearLayout);
            }
...
//            listContainer.add(linearLayout);
        ...
        for (int i = 0; i < lineCount; i++) {
            ...
            for (int j = 0; j < listLineViews.size(); j++) {
                ...
                com.my.utils.Y.y("自定义view: " + (i + 1) + " 行" + (j + 1) + "列_cLeft:" + cLeft + "left:" + left);
...
//                com.my.utils.Y.y("自定义view: " + (i + 1) + " 行" + (j + 1) + "列_left:" +left);
            }
            left = 0;
            top += lineHeight;
        }
